...
def inverse(matrix):
    ...
    # Type and Value Checks, check if Matrix is singular
    if not isinstance(matrix, list) or len(matrix) == 0 or \
            not all(isinstance(row, list) for row in matrix):
        raise TypeError("matrix must be a list of lists")
    if not (all(len(row) == len(matrix) for row in matrix)) or not \
    ...
    if determinant(matrix) == 0:
        return None
    # start of inverse caluclations
    inv = []
    new_row = []
    # 1x1 matrix
    if len(matrix[0]) == 1:
        inv.append([1 / determinant(matrix)])
        return inv

    # 2x2 matrix
    if len(matrix[0]) == 2:
        for row in range(0, len(adjugate(matrix))):
            for column in range(0, len(adjugate(matrix)[0])):
                new_row.append((1 / determinant(matrix)) *
                               adjugate(matrix)[row][column])
            inv.append(new_row)
            new_row = []
        return inv
